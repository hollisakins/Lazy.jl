...
@main function main(
    ...
    if haskey(param, "io")
        ...
        if haskey(io, "input_catalog")
            ...
            println("Input file: $input_catalog")
            # Read the input catalog
            cat = FITS(input_catalog)
        else
            error("parameter `input_catalog` not found in the parameter file.")
        end
    else
        error("section `io` not found in the parameter file.")
    end

    IDs = read(cat[2], "ID")
    nobj = length(IDs)
    
    # println("Input catalog: $cat")
    ...
    if !haskey(param, "fitting")
        error("section `fitting` not found in the parameter file.")
    end

    fitting = param["fitting"]

    if !haskey(fitting, "nphot_min")
        error("parameter `nphot_min` not found in the parameter file.")
    end
    nphot_min = fitting["nphot_min"]

    if !haskey(fitting, "sys_err")
        error("parameter `sys_err` not found in the parameter file.")
    end
    ...
    if !haskey(fitting, "z_min")
        error("parameter `z_min` not found in the parameter file.")
    end
    if !haskey(fitting, "z_max")
        error("parameter `z_max` not found in the parameter file.")
    end
    if !haskey(fitting, "z_step")
        error("parameter `z_step` not found in the parameter file.")
    end
